    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 md:p-8 animate-fade-in">
      <div className="bg-white w-full max-w-6xl rounded-[48px] overflow-hidden shadow-[0_40px_100px_rgba(0,0,0,0.08)] border border-slate-100 flex flex-col lg:flex-row min-h-[750px]">
        ...
        <div className="lg:w-1/2 p-10 md:p-16 flex flex-col justify-center bg-white text-center md:text-left">
          ...
          {sent ? (
            <div className="py-20 animate-scale-in text-center">
              ...
              <p className="text-slate-500">Un responsable reviendra vers vous par email sous 24h.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-8 text-left">
              <div className="p-8 bg-slate-50 rounded-[32px] border border-slate-100">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-6 text-center md:text-left">CATEGORIES</label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {["Éponges Inox", "Plastiques HD", "Alu / Film Pro"].map(cat => (
                    <label key={cat} className="flex items-center gap-3 text-sm font-bold text-slate-700 cursor-pointer group">
                      <input type="checkbox" name="categories" value={cat} defaultChecked={true} className="w-5 h-5 accent-blue-600 rounded-lg border-slate-200" /> 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <input required name="userEmail" type="email" placeholder="EMAIL" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-blue-100 font-bold transition-all text-center md:text-left" />
                <input required name="userPhone" type="tel" placeholder="TELEPHONE" className="w-full md:col-span-2 lg:col-span-1 px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-blue-100 font-bold transition-all text-center md:text-left" />
              </div>
        ...
        <div className="lg:w-1/2 bg-blue-600 relative overflow-hidden flex flex-col text-white">
          ...
          <div className="flex-1 flex flex-col p-12 md:p-20 relative z-10 justify-center items-center text-center">
            <div className="mb-12 text-center flex flex-col items-center">
              <h4 className="text-3xl font-black uppercase tracking-tighter leading-none mb-4 text-white">SERVICE COMMERCIAL</h4>
              <div className="space-y-2 text-[12px] font-bold uppercase tracking-[0.2em] text-blue-50">
                <div className="flex items-center justify-center gap-3"><i className="fas fa-phone-alt opacity-70"></i> 06 82 33 08 20</div>
                <div className="flex items-center justify-center gap-3"><i className="fas fa-phone-alt opacity-70"></i> 05 23 32 60 48</div>
                <div className="flex items-center justify-center gap-3"><i className="fas fa-envelope opacity-70"></i> Contact@leaderpak.ma</div>
                <div className="flex items-center justify-center gap-3 mt-6 opacity-90 leading-relaxed max-w-[280px]">
                  <i className="fas fa-map-marker-alt opacity-70"></i> 43, zone industrielle sud-ouest Mohammedia
                </div>
              </div>
            </div>

            <div className="bg-white/10 backdrop-blur-md border border-white/20 p-8 rounded-[40px] text-center relative max-w-sm mb-12">
              <i className="fas fa-info-circle absolute -top-4 left-1/2 -translate-x-1/2 text-white text-3xl"></i>
              <p className="text-lg italic font-medium leading-relaxed">
                Expertise & Proximité : Votre partenaire de confiance pour le sourcing industriel au Maroc.
              </p>
            </div>
